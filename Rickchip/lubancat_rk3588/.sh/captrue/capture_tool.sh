...
select opt in "${options[@]}"; do
    case "$opt" in
        ...
        "自動設定")
            v4l2-ctl -d /dev/v4l-subdev2 --set-ctrl "gain_automatic=0,auto_exposure=1,analogue_gain=128,exposure=100,digital_gain=1024"	
            sleep 2
            v4l2-ctl -d /dev/v4l-subdev7 --set-ctrl "gain_automatic=0,auto_exposure=1,analogue_gain=128,exposure=100,digital_gain=1024"
            sleep 2
        ...
        "capture_(raw,yuv,jpg)")
            ...
            yuv_file="${ts}.yuv"
            jpg_file="${ts}.jpg"
            echo "0 = raw file, 1 = yuv file, 2 = jpg file"
            read -p "請輸入 capture type(0 , 1, 2): " val
            if [ "$val" = "0" ]; then
            v4l2-ctl -d /dev/video11 --stream-mmap --stream-count=10 --stream-to="/home/cat/sam/image/${raw_file}"
            fi
            if [ "$val" = "1" ]; then
            sudo v4l2-ctl -d /dev/video11 --set-fmt-video=width=400,height=400,pixelformat=NV12 --stream-mmap=3 --stream-count=1 --stream-to="/home/cat/sam/image/${yuv_file}"
            fi
            if [ "$val" = "2" ]; then
            sudo v4l2-ctl -d /dev/video11 --set-fmt-video=width=400,height=400,pixelformat=NV12 --stream-mmap=3 --stream-count=1 --stream-to="/home/cat/sam/image/${yuv_file}"
            ffmpeg -f rawvideo -pixel_format nv12 -video_size 400x400 -i "/home/cat/sam/image/${yuv_file}" -frames:v 1 "/home/cat/sam/image/${jpg_file}"
            fi
            ;;       
